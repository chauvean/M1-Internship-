...
def variance_array(X, serie_length, nb_samples):
    var_array = numpy.zeros(len(X[0]))
    for j in range(serie_length):
        var_array[j] = numpy.var([X[i][j] for i in range(nb_samples)])
    return var_array
...
def main():
    ...
    serie_length = 7
    nb_samples = 2
    var_array = variance_array([ts1, ts2], serie_length, nb_samples)
    relevant_values = [-1 for _ in range(serie_length)]
    bary_array = euclidean_barycenter([ts1, ts2])
    treshold = 0.05
    for i in range(serie_length):
        if var_array[i] < treshold:
            relevant_values[i] = bary_array[i][0]
    print("bary results : {}".format(bary_array))
    print("variances : {}".format(var_array))
    print("relevant values : {}".format(relevant_values))
