...
def _check_no_empty_cluster(labels, n_clusters):
    ...
    # labels are indicating the index of a sample's cluster. If one label isnt there, no sample belongs to the cluster.
    Examples
    --------
...
def _check_full_length(centroids):
    ...
    # centroids are the correct time series format used by tslearn ( convert our raw array to correct format to get usable centroids).
    Examples
    --------
...
def silhouette_score(X, labels, metric=None, sample_size=None, metric_params=None,
    ...
    **kwds : optional keyword parameters
    ...
    """
    # coeff between -1 and 1 evaluating proximity from a sample to its own cluster and the others
    sklearn_metric = None
...
class GlobalAlignmentKernelKMeans(BaseEstimator, ClusterMixin):
    ...
    # from a ndarray, generates a kmeans clustering. We can choose nb_clusters, nb_iterations
    ...
    def fit(self, X, y=None, sample_weight=None):
        ...
        # from infos given when GlobalAlignmentKernelKMeans(BaseEstimator, ClusterMixin) initializes , computes the algorithm
        n_samples = X.shape[0]
    ...
    def _compute_dist(self, K, dist):
        ...
        for j in range(self.n_clusters):
            ...
            if numpy.sum(mask) == 0:  # means there is at least one empty cluster
...
class TimeSeriesKMeans(BaseEstimator, ClusterMixin, TimeSeriesCentroidBasedClusteringMixin):
    ...
    def __init__(self, n_clusters=3, max_iter=50, tol=1e-6, n_init=1, metric="euclidean", max_iter_barycenter=100,
        ...
        # k means adapted to time series (implements sametype of func as previous classes
        self.n_clusters = n_clusters
        self.max_iter = max_iter
        self.tol = tol
        self.random_state = random_state
...
def main():
    # labels = numpy.array([0, 1, 1, 2, 3, 4, 5, 5])
    # _check_no_empty_cluster(labels, 5)

    # centroids = to_time_series_dataset([[1, 2, 3], [1, 2, 3, 4, 5]])
    # print(_check_full_length(centroids))

    X = random_walks(n_ts=50, sz=32, d=1)
    gak_km = GlobalAlignmentKernelKMeans(n_clusters=3, verbose=False, random_state=0).fit(X)
    print(numpy.alltrue(gak_km.labels_ == gak_km.predict(X)))
    print(numpy.alltrue(gak_km.fit(X).predict(X) == gak_km.fit_predict(X)))
    GlobalAlignmentKernelKMeans(n_clusters=101, verbose=False, random_state=0).fit(X).X_fit_ is None
